...
void loop()
{
  // Фильтр "микрошума": подтверждаем движение только после нескольких подряд срабатываний.
  constexpr uint8_t REQUIRED_HITS = 3;
  ...
  if(!motionActive && hitCount >= REQUIRED_HITS){
    motionActive = true;
    Serial.println("Обнаружено движение (фильтр: игнорируем микродвижения).");
  }else if(motionActive && hitCount == 0){
    motionActive = false;
