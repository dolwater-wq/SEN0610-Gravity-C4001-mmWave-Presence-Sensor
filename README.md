...
- `examples/storozhListen4Sensors.ino` — прослушивание 4 датчиков (RX/TX: 2/1, 3/4, 5/6, 11/10) с выводом сырого UART потока в Serial Monitor; с software UART работает параллельно, без него автоматически включает fallback-режим (S3/S4 поочередно через один UART).

- `examples/zorkiyGlaz.ino` — «Зоркий глаз»: быстрое последовательное прослушивание 4 датчиков по кругу (1/2, 4/3, 6/5, 10/11) при постоянном питании; расшифровывает в человекочитаемый TARGET-вывод (цель, расстояние в метрах, скорость в м/с, энергия) и имеет авто-попытку swap RX/TX для молчащего канала.
